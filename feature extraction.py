from joining_tables import join_train_addresses, return_shipments
...
class FeatureExtractor():
    ...
    def __init__(self, path='./'):
        ...
        self.categorical = ['platform', 'os', 'retailer', 's.order_state', 'shipment_state', 's.city_name', 'is_rated', 'dw_kind']
    ...
    def collect_orders(self):
        ...
        orders = train[['phone_id', 'id']].drop_duplicates().merge(shipments,
                                                                      left_on='id', right_on='ship_address_id',
                                                                      how='right')

        orders['month'] = pd.to_datetime(shipments['order_created_at']).dt.month
        orders['year'] = pd.to_datetime(shipments['order_created_at']).dt.year
        ...
        return orders[(orders['year'] == 2020) | ((orders['year'] == 2019) & (orders['month'] == 12))]
